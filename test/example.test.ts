...
test('preserves entry input order by default', async () => {
  ...
  expect(result.indexOf('<title>First Entry</title>')).toBeLessThan(
    result.indexOf('<title>Newest Entry</title>')
  );
  expect(result.indexOf('<title>Newest Entry</title>')).toBeLessThan(
    result.indexOf('<title>Middle Entry</title>')
  );
});
...
test('can sort entries by updated descending when requested', async () => {
  ...
  expect(result.indexOf('<title>Middle Entry</title>')).toBeLessThan(
    result.indexOf('<title>First Entry</title>')
  );
});
...
test('accepts Atom URI references and non-URL URIs for Atom fields', async () => {
  ...
  expect(result).toContain(
    '<generator uri="urn:example:generator">Test Generator</generator>'
  );
