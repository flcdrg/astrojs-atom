...
export type AtomContributor = UnwrapArray<
  z.infer<typeof atomSchema>['contributor']
>;
...
function createXhtmlDiv(parser: XMLParser, value: string) {
  ...
  const childKeys = Object.keys(parsedFragment).filter(
    (key) => key !== '#text' && !key.startsWith('@_')
  );
  const textValue =
    typeof parsedFragment['#text'] === 'string'
      ? parsedFragment['#text'].trim()
      : '';
  ...
  if (childKeys.length === 1 && textValue.length === 0) {
    ...
    if (localName === 'div') {
      ...
      if (divValue && typeof divValue === 'object') {
        ...
        const prefix = onlyChildKey.includes(':')
          ? onlyChildKey.slice(0, onlyChildKey.indexOf(':'))
          : undefined;
...
function serializeSource(source: NonNullable<AtomSource>, parser: XMLParser) {
  ...
  if (source.category)
    serializedSource.category = serializeCategories(source.category);
  if (source.contributor) serializedSource.contributor = source.contributor;
  if (source.generator)
    serializedSource.generator = serializeGenerator(source.generator);
  if (source.icon) serializedSource.icon = source.icon;
  if (source.logo) serializedSource.logo = source.logo;
  if (source.rights)
    serializedSource.rights = serializeTextConstruct(source.rights, parser);
  if (source.subtitle)
    serializedSource.subtitle = serializeTextConstruct(source.subtitle, parser);
...
function serializeContentConstruct(
  ...
  if (
    ...
    normalizedType === 'text' ||
    normalizedType === 'html' ||
    isTextMediaType(normalizedType)
  ) {
    return {
      ...xmlAttributes,
      '#text': value,
...
export async function getAtomResponse(
  atomOptions: AtomFeedOptions
): Promise<Response> {
...
export async function getAtomString(
  ...
  const { stylesheet, xmlns, lang, sortEntriesByUpdated, ...schemaOptions } =
    atomOptions;
...
async function generateAtom(
  atomOptions: z.infer<typeof atomSchema> & {
    stylesheet?: string | boolean;
    xmlns?: Record<string, string>;
  ...
  root.feed.entry = entries.map((entry) => {
    ...
    if (entry.content) {
      e.content = serializeContentConstruct(entry.content, parser);
    }
    if (typeof entry.customData === 'string') {
      Object.assign(
        e,
        parser.parse(`<entry>${entry.customData}</entry>`).entry
      );
    }
    ...
    if (entry.thumbnail) {
      ...
      if (width !== undefined)
        (e['media:thumbnail'] as Record<string, any>)['@_width'] = width;
      if (height !== undefined)
        (e['media:thumbnail'] as Record<string, any>)['@_height'] = height;
      if (time !== undefined)
        (e['media:thumbnail'] as Record<string, any>)['@_time'] = time;
      ...
      if (width !== undefined)
        (e['media:content'] as Record<string, any>)['@_width'] = width;
      if (height !== undefined)
        (e['media:content'] as Record<string, any>)['@_height'] = height;
    }
